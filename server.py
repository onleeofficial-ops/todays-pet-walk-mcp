...
def validate_common_inputs(
    ...
    """반려견 이름과 지역 입력값을 검증합니다."""

    if not dog_name or not dog_name.strip():
        return {
            "status": "invalid_request",
            "message": "반려견 이름을 입력해 주세요.",
        }

    if not location or not location.strip():
        return {
            "status": "invalid_request",
            "message": "지역명을 입력해 주세요.",
        }
...
def check_server() -> dict[str, Any]:
    ...
    return {
        ...
        "message": (
            "Today's Pet Walk MCP server is operating normally."
        ),
    }
...
@mcp.tool(
    ...
    description=(
        ...
        "including the recommended place, reason, duration, "
        "and safety caution."
    ),
    annotations=ToolAnnotations(
...
def recommend_walk(
    ...
    if location_data is None:
        return {
            ...
            "message": (
                f"현재 데모 버전에서는 {location}의 장소 데이터가 "
                "준비되지 않았습니다."
            ),
    ...
    return {
        ...
        "summary": (
            ...
            f"{recommended_place['recommended_duration']} 정도 "
            "산책하는 것을 추천합니다."
        ),
        "data_notice": (
...
def find_pet_friendly_rest_place(
    ...
    if location_data is None:
        return {
            ...
            "requested_location": location,
            "message": (
                f"현재 데모 버전에서는 {location}의 실내 휴식 장소 "
                "데이터가 준비되지 않았습니다."
            ),
            "supported_locations": list(PLACE_DATA.keys()),
        }
    ...
    return {
        ...
        "data_notice": (
            "현재 결과는 공모전 데모용 데이터입니다. "
            "운영시간과 반려동물 동반 가능 여부는 방문 전에 확인하세요."
        ),
    }
...
@mcp.tool(
    ...
    description=(
        ...
        "Use this tool to help the user prepare for injuries, "
        "abnormal symptoms, or emergencies that may occur during a dog walk."
    ),
    annotations=ToolAnnotations(
